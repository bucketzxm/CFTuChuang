...
function save_new_db($fileaddress,$db){
	$fp = fopen($fileaddress, 'w+') or die("无法打开".$fileaddress);
...
function folder_nw($path){
	user_note('目录存在但是没有权限写入。<br />
		请检查PHP程序是否有权限写入 <b>'.$path.'</b> 目录。
		Linux服务器可以CHMOD目录为777 (rwxrwxrwx)。<br/>然后再重新加载本页面。',1);
}
function folder_nc($path){
	user_note('无法创建目录。<br />
		请检查服务器中存在 <b>'.$path.'</b> 目录，并且PHP程序有权限写入。
		Linux服务器可以CHMOD目录为777 (rwxrwxrwx)。<br/>然后再重新加载本页面。',1);
}
