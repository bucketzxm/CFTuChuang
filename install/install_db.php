 ...
 *************************************************************************************************************/
	...
	// Image Database
		echo '<div class="ibox"><h2>检查图片数据库</h2>';
		if (!file_exists(CFDBIMGPATH)){
			user_note('未找到图片数据库，正在新建数据库...',3);
			false;
	...
	// Bandwidth Database
		if($check_bw){
			echo '<div class="ibox"><h2>检查带宽数据库</h2>';
			$bw_files = glob(CFBANDWIDTHPATH . "*.db");

			if(count($bw_files) > 0){
				user_note('找到 '.count($bw_files).' 带宽数据库，正在检查是否需要更新....',3);
				flushNow();
				// var counters
				...
				user_note(''.$u.' 文件已更新，同时 '.$r.' 文件已移除。',2);
			}else{
				user_note('未找到带宽数据库',3);
			}
			echo '</div>';
			flushNow();
		}
	...
	// move on
		if(!$Err_found){
			echo '<form method="post" action="?i=set" class="ins">
				<input type="submit" value="下一步：检查设置" class="button" name="db">
			</form>';
		}else{
			echo '<div class="ibox">';
