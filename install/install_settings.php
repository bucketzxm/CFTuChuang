 ...
 *************************************************************************************************************/
	// make settings
			echo '<div class="ibox"><h2>设置</h2>';
			
			// chech for seeings file
				if(!is_file(CFROOTPATH.'inc/set.php')){
					user_note('未找到设置文件，正在新建设置文件',3);
				}else{
					user_note('找到设置文件，正在更新...',3);
					$update = true;
					define('CFIHP', 'installer');
					include_once(CFROOTPATH.'inc/set.php');
				}
			...
			if(!saveSettings(CFROOTPATH.'inc/set.php',$content)){
				user_note('创建设置文件时出错！',1);
				$Err_found = true;
			}else{
		...
		// Remove old files
			...
			if(isset($removeOldFile)){
				...
				foreach ($removeOldFile as $oldFile){
					if(@unlink(CFROOTPATH.$oldFile)) user_note('已删除：<b>'.$oldFile.'</b><br/>',2);
				}
				echo '</div>';
			}
